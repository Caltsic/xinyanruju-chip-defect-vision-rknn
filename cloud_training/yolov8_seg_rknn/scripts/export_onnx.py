#!/usr/bin/env python3
"""Export trained YOLOv8-seg weights to a stable-named ONNX model."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--weights", required=True, type=Path, help="Trained YOLOv8-seg .pt file.")
    parser.add_argument("--output", required=True, type=Path, help="Stable output ONNX path.")
...
def export_with_standard_ultralytics(args: argparse.Namespace) -> Path:
    ...
    exported = model.export(format="onnx", task="segment", imgsz=args.imgsz, opset=args.opset, simplify=True, dynamic=False, nms=False)
