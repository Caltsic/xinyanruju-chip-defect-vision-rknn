#!/usr/bin/env python3
"""Prepare the one-class chip ROI dataset for YOLOv8 training.

The source dataset is already YOLO detection format. This script validates
pairing and labels, writes a fresh `data.yaml`, and creates the calibration
image list used by RKNN INT8 conversion.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", required=True, type=Path, help="Output YOLOv8 dataset root.")
...
def main() -> None:
    ...
    print(f"Prepared chip ROI dataset: {stats.output}")
    print(f"Images: {total_images}, objects: {total_objects}")
    for split, split_stats in stats.splits.items():
        print(
            f"{split}: images={split_stats.images}, labels={split_stats.labels}, "
            f"objects={split_stats.objects}, empty_labels={split_stats.empty_label_files}"
        )
